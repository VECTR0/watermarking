...
class BlurAttacker(Attacker):
    ...
    def attack(self, dto: Dto) -> AttackingResults:
        ...
        blurred_image = cv2.GaussianBlur(dto.image, kernel_size, 0)
...
class JpegCompressionAttacker(Attacker):
    ...
    def attack(self, dto: Dto) -> AttackingResults:
        ...
        _, encoded_image = cv2.imencode(".jpg", dto.image, encode_param)
...
class NoiseAdditionAttacker(Attacker):
    ...
    def attack(self, dto: Dto) -> AttackingResults:
        ...
        noise = np.random.normal(0, self.intesity, dto.image.shape).astype(np.float32)
        noisy_image = np.clip(dto.image + noise, 0, 255).astype(np.uint8)
        return noisy_image
...
#         h, w, _ = dto.image.shape
#         cropped_image = dto.image[h // 4 : 3 * h // 4, w // 4 : 3 * w // 4]
...
#         h, w, _ = dto.image.shape
#         scaled_image = cv2.resize(dto.image, (w // 2, h // 2))  # Skalowanie w dół
...
class WarpingAttacker(Attacker):
    ...
    def attack(self, dto: Dto) -> AttackingResults:
        ...
        warped_image = cv2.warpAffine(dto.image, matrix, (w, h))
        return warped_image
...
#         overlayed_image = cv2.addWeighted(dto.image, 0.5, sample, 0.5, 0)
...
class OverlayAttacker(Attacker):
    ...
    def attack(self, dto: Dto) -> AttackingResults:
        ...
        overlay = np.zeros_like(dto.image)
        h, w, _ = dto.image.shape
        cv2.rectangle(
            overlay, (w // 4, h // 4), (3 * w // 4, 3 * h // 4), (255, 0, 0), -1
        )
        overlayed_image = cv2.addWeighted(dto.image, 0.7, overlay, 0.3, 0)
        return overlayed_image
